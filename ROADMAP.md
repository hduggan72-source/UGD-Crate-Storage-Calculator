...
- ADD MANUAL OPTION FOR ENTERING WOVEN GEOTEXTILE OUTSIDE OF PT-ROW CALCULATOR (OPTION TO DOUBLE AMOUNT FOR TWO LAYER WRAP)
...
- CLONE TANK IN MULTI
- MOVE PROJECT INFOR FOR QUOTE TO LEFT PANEL RIGHT BELOW THE TANKS IN PROJECT. THIS WILL ALLOW FOR MORE OPEN AREA FOR READING AND ENTERING THE TANK INPUTS AND SUMMARY DATA
- HAVE THE CUMULATIVE BOM BELOW THE HEADER AS LIKE THE TOP OF THE DASHBOARD WHICH REMAINS STATIC
- ***'REMOVE COMPLEX TANK CALCULATOR FROM CLIENT FACING TOOL????*** PROS/CONS???

##UPDATES FOR DESIGN TOOLS PAGE

- REVIEW ALL DISCLAIMERS AND UPDATE DISCALIMER ON PT-ROW AS IT REFERENCES OEPA DETAIL
- ENSURE THE AMOUNT OF WOVEN FABRIC AND NON-WOVEN GEO IS CONSISTENT WITH SINGLE & MULTI TANK CALCULATORS
  - 2 LAYERS OF WOVEN SEEMS TO BE DEFAULT ON DESIGN TOOL

- UPDATE COMPETITOR SYSTM MAKING IT MORE GENERIC UNTIL WE CAN PRELOAD ACTUAL COMPETITOR SPECS AND HAVE A DROPDOWN BOX
- If your tool has an alternate option where “module height” means total competitor tank height, RATHER THAN INDIVIDUAL MODULE HEIGHT X LAYERS THAT MIGHT BE HELPFUL. ALTHOUGH, SOMETIMES THEY WILL PIECEMEAL TWO DIFFERENT MODULE TYPES TOGETHER TO ACHIEVE A SPECIFIC HEIGHT DUE TO PROJECT SITE OR VOLUME CONSTRAINTS.
- ALSO NEED TO UPDATE AQUACELL MODULE VOLUMES PER LAYER BY HAVING IT PULL FROM THE AQ-200-02 WORKSHEET SO IT DIALS IN AN ACCURATE STORAGE VOLUME AND CREATE COUNT
...
- (NOW) SPEC BUILDER PAGE - TO REPLACE ME HAVING TO DO ALL THE "STANDARD" TANK DESIGNS (COMES OUT WITH CONCEPTUAL SCHEMATIC ON PLAN OVERLAY; AND HAS THE DETAILS THAT CAN BE DOWNLOADED INCLUDING SPEC SHEETS FOR CIVEL PAGES ALREADY BUILT)
...
- **ON MIN. DISTANCE FROM STRUCTURE CALCULATOR IN DESIGN TOOLS, 0 & 90 DEGREES FOR SOIL WEDGE IS THROWING AN ERROR WHEN THE ANGLE MUST BE 0 AND 90 DEGREES**
- **OLD BUG — CRATE COMPARISON PANEL ON DESIGN TOOLS PAGE SHOWS UP ALONGSIDE THE DEFAULT/ACTIVE CALCULATOR ON PAGE LOAD (SQUEEZES INTO THE LAYOUT AS AN EXTRA COLUMN) UNTIL YOU CLICK ANY OTHER CALCULATOR TAB, THEN IT DISAPPEARS. CAUSE: `panel-crate_comparison` IS MISSING `style="display:none"` THAT EVERY OTHER NON-DEFAULT PANEL HAS. ONE-LINE FIX (ALREADY APPLIED ON THE CLIENT-FACING TRIM BRANCH) — JUST NEEDS PORTING HERE.**
...
-  [X] SEPARATE PAGE FOR "DESIGN TOOLS" MULTIPLE CALCULATORS LIVE AND EXPORT TO THEIR OWN PAGE WITH GRAPHICS
   ...
   - [X] UPLOAD PLAN SHEET AND CREATE AN OVERLAY OF TANK (RECTANGLE/SQUARE ONLY)
   - [X] ADD ZOOM IN/OUT FEATURE TO THE SITE OVERLAY PAGE (MAKE IT EASIER TO SCALE ON A LARGE PAGE)
   - [X] PT-ROW™ DESIGN CALCULATOR
    - [ ] WITH VISUAL WITHIN A STRUCTURED TANK (OR BE ABLE TO PLACE BASED ON NUMBER OF CALCULATED CRATES)
    - [ ] WHEN SPECIFIC SIZING IS USED, ONLY SHOW THE RESULT OF THE OPTION ON THE SUBMITTAL PDF

- **## PREVIOUS Version (v12-v15) — Completed**

-  [X] ADD PROJECT NAMES TO EXPORTED REPORTS FROM DESIGN TOOLS PAGE
-  [X] DRAG & DROP JSON FILE ENABLED FOR BOTH UI'S
-  [X] ADD FREIGHT ESTIMATE LINE TO THE CS EXPORT $***
-  [X] ADD FREIGHT TO CS EXPORT FROM MULTI-TANK UI AS WELL
-  [X] ADD SITE ADDRESS LINE TO MULTI-TANK UI AS WELL
