...
def index():
    ...
    if request.method == 'POST':
        # Read form data with safe conversions
...
def download_pdf():
    # Read all form data with safe conversions
    ...
    lines = [
        ...
        f"Minimum Required Storage: {min_storage if min_storage is not None else '—'} ft³",
        f"Storage Check: {storage_status}",
        "",
        "Geotextile Fabric (Non-woven - Burrito Style Wrap)",
        f"Geotextile - AquaCell Only: {geo_tank:.1f} ft²",
        f"Geotextile - Stone Envelope: {geo_stone:.1f} ft²",
        f"Total Geotextile: {geo_total:.1f} ft²",
        f"Waste/Overlap: {geo_waste:.0f}%",
        "",
        "Bill of Materials",
    ...
    disclaimer = (
        ...
        "all design, calculations, soil bearing, compaction, and site-specific conditions."
    )
