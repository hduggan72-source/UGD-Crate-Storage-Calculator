...
def index():
    ...
    if request.method == 'POST':
        ...
        results = {
            ...
            'known_width': known_width, 'known_length': known_length,
            'tank_width': round(tank_width, 2), 'tank_length': round(tank_length, 2), 'tank_height': round(tank_height, 2),
            ...
            'geoTank': round(geo_tank, 1), 'geoStone': round(geo_stone, 1), 'geoTotal': round(geo_total, 1),
            'geoWaste': geo_waste,
...
def download_pdf():
    # === FULL RECALCULATION (identical to index) ===
    ...
    # Tank & Storage calculations (same as index)
    MODULE_WID = 1.9685
    MODULE_LEN = 3.937
    crates_wide = math.floor(known_width / MODULE_WID)
    crates_long = math.floor(known_length / MODULE_LEN)
